    ...
    <div class="container">
        <div style="width:50%; padding-right : 30px; float:left;">
            ...
            <div class ="input-group" style="margin-bottom:10px;">
                <input type="text" class="form-control" placeholder="이름" v-model="name" @keyup.enter ="createUser(name)">
                <input type="text" class="form-control" placeholder="나이" v-model="age" @keyup.enter ="createUser(age)">
                <input type="text" class="form-control" placeholder="성별" v-model="sex" @keyup.enter ="createUser(sex)">
            </div>
            ...
            <div v-if="isModify">
                ...
                <input type="text" class="form-control" placeholder="나이" v-model="modifyAge" @keyup.enter ="modifyUser(modifyAge)">
                <input type="text" class="form-control" placeholder="성별" v-model="modifySex" @keyup.enter ="modifyUser(modifySex)">
                <button class="btn btn-success" type="button" @click="modifyUser(modifyStr)"> 수정완료 </button>
            </div>
            <br>
 
            <ul class="list-group">
                <li class="list-group-item" v-for="(user, index) in pageUsers[curpageNumber]" :key="index">
                    <strong>이름 : </strong>{{ user.name }} <strong>　　나이 : </strong> {{ user.age }} <strong>　　성별 : </strong> {{ user.sex }}
        ...
        <div style="width:50%; padding-left : 30px; float:right;">
            <div class="btn-group">
                <button type="button" class="btn btn-primary" @click="searchOn">검색</button>
                <button type="button" class="btn btn-primary" @click="filterOn">필터링</button>
            </div>
            <div v-if="isSearch">
                <h2> Search </h2>
                <div class ="input-group" style="margin-bottom:10px;">
                    <input type="text" class="form-control" placeholder="이름" v-model="searchName" @keyup.enter ="searchUser(searchName)">
                    <input type="text" class="form-control" placeholder="나이" v-model="searchAge" @keyup.enter ="searchUser(searchAge)">
                    <input type="text" class="form-control" placeholder="성별" v-model="searchSex" @keyup.enter ="searchUser(searchSex)">
                </div>
                ...
                <ul class="list-group">
                    <li class="list-group-item" v-for="(user, index) in searchUsers" :key="index">
                        <strong>이름 : </strong>{{ user.name }} <strong>　　나이 : </strong> {{ user.age }} <strong>　　성별 : </strong> {{ user.sex }}
                    </li>
                </ul>
            </div>
            <div v-if="!isSearch">
                ...
                <ul class="list-group">
                    <li class="list-group-item" v-for="(user, index) in filterUsers" :key="index">
                        <strong>이름 : </strong>{{ user.name }} <strong>　　나이 : </strong> {{ user.age }} <strong>　　성별 : </strong> {{ user.sex }}
                    </li>
                </ul>
            </div>
